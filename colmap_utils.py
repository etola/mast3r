...
class ColmapReconstruction:
    ...
    def get_best_pair_for_image(self, target_image_id: int, max_pairs: int = 5, min_points: int = 100, 
                               parallax_sample_size: int = 100, min_feature_coverage: float = 0.6) -> List[int]:
        """
        Find the best matching images for a specific target image.
        
        Args:
            target_image_id: ID of the image to find partners for
            max_pairs: Maximum number of partner images to return
        ...
        Returns:
            List of partner image IDs sorted by quality (best first), up to max_pairs length
        """
        ...
        # Return top candidates with good parallax (> 0.1 radians ≈ 5.7 degrees)
        good_candidates = []
        for candidate in match_candidates:
            if candidate.avg_parallax_angle > 0.1 and len(good_candidates) < max_pairs:
                good_candidates.append(candidate.image_id)
        
        # If no good parallax candidates, just take the top ones by shared points/coverage
        if len(good_candidates) == 0:
            good_candidates = [candidate.image_id for candidate in match_candidates[:max_pairs]]
